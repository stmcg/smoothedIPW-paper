...
beta_L <- c(-1.5, 0.25, 0.25, 0.5, 0.1)
beta_Z <- c(0.5, -1)
beta_A <- c(1.75, 0.5, -0.25, 0.5, -0.1)
beta_R <- c(-1, 0.5, 0.25, -0.5)
beta_Y <- c(0, 2, -3, -2, 2, 3, -1, 3, 0.25)
sigma_Y <- 5
U_lb <- -1; U_ub <- 1
m <- 2
...
results_true <- foreach(rep = 1:n_reps_true) %dorng% {
  df_z0 <- lapply(as.list(1:n), FUN=function(ind){
    datagen_true(i = ind, time_points = time_points,
                 beta_L = beta_L, Z = 0, beta_A = beta_A, beta_Y = beta_Y, 
  ...
  df_z1 <- lapply(as.list(1:n), FUN=function(ind){
    datagen_true(i = ind, time_points = time_points,
                 beta_L = beta_L, Z = 1, beta_A = beta_A, beta_Y = beta_Y, 
                 sigma_Y = sigma_Y, U_lb = U_lb, U_ub = U_ub, m = m)
  })
  ...
  for (i in 1:length(outcome_times)){
    true_z0[i] <- mean(df_z0[df_z0$time == outcome_times[i], ]$Y)
    true_z1[i] <- mean(df_z1[df_z1$time == outcome_times[i], ]$Y)
  }
...
save.image('../results/res-nonnull.RData')
